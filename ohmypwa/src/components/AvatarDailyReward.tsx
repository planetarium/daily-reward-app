...
async function makeDailyRewardTransaction(
  account: Account,
  nonce: bigint,
...
export const AvatarDailyReward: React.FC<AvatarDailyRewardProps> = ({
  ...
  const [stageTransaction, { data, loading, error }] =
  ...
  return (
    ...
    >
      ...
      {claimable ? (
        ...
        <p>Wait for {blocksToWait} blocks.</p>
      )}
    </div>
  );
};
