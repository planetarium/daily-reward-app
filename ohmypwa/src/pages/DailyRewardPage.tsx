...
export const DailyRewardPage: React.FC<DailyRewardPageProps> = ({ account, address }) => {
    ...
    return (
        <div>
            ...
            <p>{address}</p>
            <h3>Avatars</h3>
            {data.stateQuery.agent?.avatarStates?.map(x => <AvatarDailyReward key={x.address} {...x} tip={tip} nonce={nonce} account={account} />)}
        </div>
    )
};
